...
def _add_slide_cover(prs, layout_cover, company_name):
    """Añade la diapositiva de portada corporativa."""
    slide = prs.slides.add_slide(layout_cover)
    remove_slide_placeholders(slide)

    title_box = slide.shapes.add_textbox(Inches(0.5), Inches(2.2), Inches(12.333), Inches(3.0))
    ...
    footer_box = slide.shapes.add_textbox(Inches(0.5), Inches(6.2), Inches(12.333), Inches(0.8))
...
def _add_slide_module_co_ps(prs, layout_content, complexity, rows, cols, left_t, top_t, width_t, height_t):
...
def _add_slide_customer(prs, layout_content, sector, description):
    ...
    remove_slide_placeholders(slide)
    add_header(slide, "Entendimiento del Cliente y Contexto de Mercado", f"Operaciones en el Perú y tendencias clave en el sector: {sector}")
    
    card1 = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), Inches(1.8), Inches(5.9), Inches(4.8))
    card1.fill.solid()
    ...
    p_c1_body.font.size = Pt(11) if len(description) > 220 else Pt(12)
    p_c1_body.font.color.rgb = COLOR_TEXT
    
    card2 = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.933), Inches(1.8), Inches(5.9), Inches(4.8))
    card2.fill.solid()
...
def _add_slide_pains(prs, layout_content):
    ...
    pain_titles = [
        "1. Logística y Abastecimiento", "2. Gestión Contable y Financiera", "3. Control de Gestión Interno"
    ]
    pain_desc = [
    ...
    for i in range(3):
        ...
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left_pos, Inches(1.8), card_width, card_height)
        card.fill.solid()
...
def _add_slide_grow(prs, layout_content):
    ...
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "La Solución Estratégica: SAP S/4HANA Public Cloud", "Acelerando el crecimiento operativo con mejores prácticas preconfiguradas")
    
    box_left = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), Inches(1.8), Inches(5.9), Inches(4.8))
    box_left.fill.solid()
    ...
    box_right = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.933), Inches(1.8), Inches(5.9), Inches(4.8))
    ...
    p_br_b.text = ("\n\n• Experiencia de Usuario Moderna (Fiori): Procesos limpios e intuitivos que reducen drásticamente la curva de aprendizaje de los colaboradores.\n\n"
                   "• Inteligencia Artificial (SAP Joule): Respuestas automáticas y comandos directos que eliminan la navegación engorrosa en menús complejos.\n\n"
                   "• Trazabilidad Total: Trazabilidad contable instantánea desde la Solicitud de Pedido (SolPed) hasta el pago de la factura al proveedor.")
...
def _add_slide_fi_mm(prs, layout_content):
    ...
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "Alcance Funcional Parte I: Procesos Core (FI & MM)", "Detalle de los módulos de Finanzas y Gestión de Materiales incluidos en el alcance")
    rows = 3
    cols = 3
    left_t = Inches(0.5)
    top_t = Inches(1.8)
...
def _add_slide_efficiency(prs, layout_content, left_t, top_t, width_t, height_t):
    """Añade diapositiva de comparación de eficiencias."""
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "Casos de Uso Prácticos / Matriz de Éxito", "Operación Tradicional vs Eficiencia en el Sistema SAP S/4HANA (Joule)")
    rows_comp = 4
    cols_comp = 2
    table_shape3 = slide.shapes.add_table(rows_comp, cols_comp, left_t, top_t, width_t, height_t)
...
def _add_slide_change(prs, layout_content):
    ...
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "Gestión del Cambio y Cultura Organizacional", "Metodología de acompañamiento y adopción para el equipo de colaboradores")
    
    box_adop = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), Inches(1.8), Inches(5.9), Inches(4.8))
    box_adop.fill.solid()
    ...
    p_ba_b.text = ("\n\nLa adopción tecnológica es liderada bajo un enfoque práctico:\n\n"
                   "• Capacitación Focalizada: Talleres prácticos sobre el uso del chat de Joule, enseñando reglas claras (ej: evitar ambigüedades como 'clientes morosos' y usar términos exactos como 'facturas vencidas').\n\n"
                   "• Fiori Quick-Wins: Capacitación intensiva en el portal Fiori para agilizar transacciones diarias.\n\n"
                   "• Gestión Preventiva: Identificación y mitigación proactiva de la resistencia al cambio en las fases iniciales.")
    p_ba_b.font.name = FONT_BODY
    p_ba_b.font.size = Pt(11)
    p_ba_b.font.color.rgb = COLOR_TEXT
    
    box_feed = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.933), Inches(1.8), Inches(5.9), Inches(4.8))
    box_feed.fill.solid()
    ...
    p_bf_b.text = ("\n\nEl equipo consultor de SEIDOR implementa la metodología de feedback estructurada para coordinar con los líderes:\n\n"
                   "• Describir Situación: Enfocado en hechos objetivos y medibles del proyecto SAP, evitando juicios.\n\n"
                   "• Impacto en Negocio: Explicar cómo afecta al cronograma o equipo (ej. retraso de FI afecta integrales).\n\n"
...
def _add_slide_economics(prs, layout_content, summary):
    ...
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "Propuesta Económica e Inversión Localizada", f"Desglose de inversión bimoneda aplicable en el Perú (Tipo de Cambio: {summary['tipo_cambio_pen']})")
    
    width_tf = Inches(6.8)
    height_tf = Inches(4.5)
    rows_f = 7
    cols_f = 3
    table_shape_f = slide.shapes.add_table(rows_f, cols_f, Inches(0.5), Inches(1.8), width_tf, height_tf)
    ...
    chart_shape = slide.shapes.add_chart(XL_CHART_TYPE.PIE, Inches(7.5), Inches(1.8), Inches(5.3), Inches(4.5), chart_data)
...
def _add_slide_roi(prs, layout_content, summary, exp_wks, real_wks, deploy_wks):
    ...
    slide = prs.slides.add_slide(layout_content)
    remove_slide_placeholders(slide)
    add_header(slide, "Cronograma y Retorno de Inversión (ROI)", "Cronograma del proyecto (SAP Activate) y análisis de recuperación financiera")
    ...
    tasks = ["Alineación y sandbox.", "Talleres de diseño y BPD.", "Configuración y pruebas.", "Migración y Go-Live.", "Soporte continuo."]
    ...
    chart_shape_col = slide.shapes.add_chart(XL_CHART_TYPE.COLUMN_CLUSTERED, Inches(0.5), Inches(3.7), Inches(8.3), Inches(3.2), chart_data_col)
    ...
    box_metrics = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(9.3), Inches(3.7), Inches(3.5), Inches(3.2))
...
def _add_slide_closing(prs, layout_closing):
    """Añade la diapositiva de cierre."""
...
def generate_deck(company_name, sector, description, complexity, financial_data, output_path):
    template_name = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "Capacitación de Joule - El futuro de SAP.pptx")
    ...
    _add_slide_cover(prs, layout_cover, company_name)
    _add_slide_customer(prs, layout_content, sector, description)
    _add_slide_pains(prs, layout_content)
    _add_slide_grow(prs, layout_content)
    _add_slide_fi_mm(prs, layout_content)
    ...
    width_t = Inches(12.333)
    height_t = Inches(4.5)
    _add_slide_module_co_ps(prs, layout_content, complexity, rows, cols, left_t, top_t, width_t, height_t)
    
    _add_slide_efficiency(prs, layout_content, left_t, top_t, width_t, height_t)
    _add_slide_change(prs, layout_content)
    _add_slide_economics(prs, layout_content, summary)
    _add_slide_roi(prs, layout_content, summary, exp_wks, real_wks, deploy_wks)
